...
def test_main_name_guard() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-c", f"exec(open('{main_file}').read().replace('if __name__', 'if True #'))"],
    ...
    async with app.run_test() as pilot:
        ...
        with patch.object(sp, "run", side_effect=FileNotFoundError("no cli")):
            with patch.object(panel, "_write_log") as mock_log:
                with patch.object(panel.app, "call_from_thread", side_effect=call_from_thread_side_effect):
